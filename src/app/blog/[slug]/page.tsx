...
export default async function BlogPostPage({
  ...
  const currentUrl = `${process.env.NEXT_PUBLIC_SITE_URL || "https://nutrafy.pk"}/blog/${post.slug}`;
  ...
  return (
    <article className="space-y-6 rounded-xl border bg-white p-6">
      ...
      <div className="prose-content text-sm text-zinc-700 whitespace-pre-line">
        {cleanContent}
      </div>
      <div className="flex gap-3 text-sm">
        ...
        <a
          href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(cleanTitle)}`}
          target="_blank"
          rel="noopener noreferrer"
          className="rounded-md border px-3 py-1"
        >
